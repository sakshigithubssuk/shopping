...
const handleFilter=(value,sectionId)=>{                                          //handle filter part
  const searchParamms = new URLSearchParams(location.search)        
...
useEffect(() => {
  // Deriving minPrice/maxPrice - seems fine
  const [minPrice, maxPrice] = priceValue === null ? [0, 10000] : priceValue.split("-").map(Number); // Increased maxPrice just in case

  // *** KEY AREA ***
  const data = {
      category: param.lavelThree,
      colors: colorValue || [], // Assumes API handles empty array or you split CSV string elsewhere
      sizes: sizeValue || [],   // Assumes API handles empty array or you split CSV string elsewhere
      minPrice,
      maxPrice,
      minDiscount: discount || 0,
      sort: sortValue || 'price_low',
      pageNumber: pageNumber || 1, // Default to 1 instead of -1. -1 might be invalid for API.
      pageSize: 10,                // **** Correctly set to 1 ****
      stock: stock,
  }
  // *** END KEY AREA ***

  // Logging the data being sent
  console.log("Dispatching findProducts with data:", JSON.stringify(data)); // <<< Add this log
  ...
  discount,
  sortValue,
  pageNumber, // <<< This is the CRITICAL dependency for pagination
  stock,
  dispatch // <<< Good practice to include dispatch
])
  return (
          ...
          <section aria-labelledby="products-heading" className="pt-6 pb-24">
            ...
            <div className="grid grid-cols-1 gap-x-8 gap-y-10 lg:grid-cols-5">
              ...
              <div>
                ...
                <h1 sr={{color:"black"}} className='text-lg opacity-50 font-bold'>Filters</h1>
                <FilterListIcon/>
                </div>
...
<div className='flex flex-wrap justify-center bg-white py-5'>
  {/* Apply optional chaining to the .map() call as well */}
  {product.products &&product.products?.content?.map((item) => <ProductCard product={item} />)}
  {/* It's also good practice to add a key prop when mapping */}
</div>
              </div>
            </div>
          </section>
